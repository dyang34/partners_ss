...
class PlanCodePriceLongTermDao extends A_Dao
{
	...
	function selectPerPage($db, $wq, $pg) {
		$sql =" select @rnum:=@rnum+1 as rnum, r.* from ("
			...
			." 		from plan_code_price_longterm a "
			." 		INNER JOIN ( "
	        ."			select no as idx from plan_code_price_longterm a "
            			.$wq->getWhereQuery()
						.$wq->getOrderByQuery()
...
*/	                
	...
	function update($db, $uq, $key) {
	    ...
	    $sql =" update plan_code_price_longterm"
			.$uq->getQuery($db)
	        ." where no = ".$this->quot($db, $key);
	...
	function delete($db, $key) {
	    if ($key) {
	    	$sql =" delete from plan_code_price_longterm where no = ".$this->quot($db, $key);
			return $db->query($sql);
		}
	}	
}
