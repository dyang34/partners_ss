...
class PlanCodePriceHanaDao extends A_Dao
{
	...
	function selectByKey($db, $key) {
		...
		$sql =" select no,member_no,trip_type,plan_code,plan_type,term_day,sex,age,price,company_type "
	...
	function selectPerPage($db, $wq, $pg) {
		$sql =" select @rnum:=@rnum+1 as rnum, r.* from ("
			...
			." 		from plan_code_price_hana a "
			." 		INNER JOIN ( "
	        ."			select no as idx from plan_code_price_hana a "
            			.$wq->getWhereQuery()
						.$wq->getOrderByQuery()
...
/*
	    $sql =" insert plan_code_price_hana(mem_type,mem_state,uid,upw,com_name,email,hphone,com_no,regdate,com_percent,com_percent_other,last_login,post_no,post_addr,post_addr_detail,fax_contact,web_site,com_open_date,etc,insuran1,insuran2,insuran3,insuran4,insuran5,insuran6,insuran7,file_real_name,file_name,insuran8,insuran9,insuran10,company_type )"
...
*/	                
	...
	function update($db, $uq, $key) {
	    ...
	    $sql =" update plan_code_price_hana"
			.$uq->getQuery($db)
	        ." where no = ".$this->quot($db, $key);
	...
	function delete($db, $key) {
	    if ($key) {
	    	$sql =" delete from plan_code_price_hana where no = ".$this->quot($db, $key);
			return $db->query($sql);
		}
	}	
}
