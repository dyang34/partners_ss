...
if(empty($company_type) || empty($member_no) || empty($CONFIG_PLAN_FILE_TRIP_TYPE) || empty($cal_type)) {
	exit;
}

$cnt_cal_type = count($__ARR_CONFIG_PLAN[$company_type]['List'][$member_no][$CONFIG_PLAN_FILE_TRIP_TYPE]);
$cnt_plan_type = count($__ARR_CONFIG_PLAN[$company_type]['List'][$member_no][$CONFIG_PLAN_FILE_TRIP_TYPE][$cal_type]);

$cal_type_text = $arrCalTypeTitle[$cnt_cal_type][$cal_type-1];
$age_from = $__ARR_CONFIG_PLAN[$company_type]['List'][$member_no][$CONFIG_PLAN_FILE_TRIP_TYPE][$cal_type][0]["plan_start_age"];
$age_to = $__ARR_CONFIG_PLAN[$company_type]['List'][$member_no][$CONFIG_PLAN_FILE_TRIP_TYPE][$cal_type][0]["plan_end_age"];
...
<?php
    ...
    for($j=0;$j<$cnt_plan_type;$j++) {
        for($k=1;$k<=34;$k++) {
            $arrPlanTypePrice[$k][$j] = $__ARR_CONFIG_PLAN[$company_type]['List'][$member_no][$CONFIG_PLAN_FILE_TRIP_TYPE][$cal_type][$j]["type_".$k."_text"];
            $arrPlanTypePrice[$k][19] .= $__ARR_CONFIG_PLAN[$company_type]['List'][$member_no][$CONFIG_PLAN_FILE_TRIP_TYPE][$cal_type][$j]["type_".$k."_text"];
        }

        $plan_type_text = $__ARR_CONFIG_PLAN[$company_type]['List'][$member_no][$CONFIG_PLAN_FILE_TRIP_TYPE][$cal_type][$j]["plan_title"];
        $plan_type_text_show = $__ARR_CONFIG_PLAN[$company_type]['List'][$member_no][$CONFIG_PLAN_FILE_TRIP_TYPE][$cal_type][$j]["plan_code"]."<br/>[".$__ARR_CONFIG_PLAN[$company_type]['List'][$member_no][$CONFIG_PLAN_FILE_TRIP_TYPE][$cal_type][$j]["plan_title"]."]";
?>
                    <li class="<?=($cnt_plan_type<2)?"plan-alone":""?>">
                        <span>
                            <?=$plan_type_text_show?>
                            <a class="button <?=($__ARR_CONFIG_PLAN[$company_type]['List'][$member_no][$CONFIG_PLAN_FILE_TRIP_TYPE][$cal_type][$j]["plan_code"]==$plan_code)?"gray":"choice"?>" name="btnChoiceMemPlan" plan_code="<?=$__ARR_CONFIG_PLAN[$company_type]['List'][$member_no][$CONFIG_PLAN_FILE_TRIP_TYPE][$cal_type][$j]["plan_code"]?>" plan_type="<?=$__ARR_CONFIG_PLAN[$company_type]['List'][$member_no][$CONFIG_PLAN_FILE_TRIP_TYPE][$cal_type][$j]["plan_type"]?>" plan_title="<?=$cal_type_text_age." ".$plan_type_text?>">선택</a>
                        </span>
                    </li>
<?
    }
                ...
                <ul class="clearfix inb tbody-tr">
                    <li>
                        <span><?=$__ARR_CONFIG_PLAN_TYPE[$company_type][$member_no][$CONFIG_PLAN_FILE_TRIP_TYPE]["type_".$k]["title"]?>
                            <div class="btn-mobal btn_plan_type_info_choice" c_title="<?=$__ARR_CONFIG_PLAN_TYPE[$company_type][$member_no][$CONFIG_PLAN_FILE_TRIP_TYPE]["type_".$k]["title"]?>" c_content="<?=$__ARR_CONFIG_PLAN_TYPE[$company_type][$member_no][$CONFIG_PLAN_FILE_TRIP_TYPE]["type_".$k]["content"]?>">
...
<?php
            for($j=0;$j<$cnt_plan_type;$j++) {
?>
                    <li class="<?=($__ARR_CONFIG_PLAN[$company_type]['List'][$member_no][$CONFIG_PLAN_FILE_TRIP_TYPE][$cal_type][$j]["plan_code"]==$plan_code)?"active":""?> <?=($cnt_plan_type<2)?"plan-alone":""?>">
